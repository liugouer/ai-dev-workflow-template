...
```
/newdev <需求主题描述>
/newdev --plan <需求主题描述>       # 仅生成需求和 Spec 预览，不执行
/newdev --yes <需求主题描述>        # 跳过确认直接创建并执行全闭环
```
...
8. 特别遵守 `.codebuddy/rules/HumanInterventionRules.mdc`
9. 从用户输入中提取需求主题
10. 判断该主题是否可以作为一个最小需求单元
11. 如果过大，拆分为多个最小需求并等待用户选择
12. 如果用户输入了 --plan，仅生成需求和 Spec 预览，不执行创建与闭环
13. 如果符合最小需求单元，生成 TAPD 标题和标准需求正文
14. 如果用户没有输入 --yes，先展示标题和正文，等待用户确认
15. 如果用户输入了 --yes，则直接创建 TAPD 需求
16. 优先使用 TAPD MCP 创建需求（create_story_or_task）
17. 创建成功后，读取新需求，获取短需求 ID 和完整 story_id
18. 自动启动该需求的完整研发闭环
19. 自动生成 Spec
20. 自动实现代码
21. 自动编写或更新测试
22. 自动运行测试和覆盖率
23. 自动执行 AI Code Review
24. 自动修复 Low / Medium 问题
25. Critical 问题必须暂停（参照 HumanInterventionRules.mdc）
26. 自动生成 Review 报告和 TAPD 归档 JSON
27. 自动 commit 和 push
28. 自动创建 MR
29. 等待 CNB 状态检查
30. 状态检查通过后自动合并 MR
31. 合并后确认 main push 流水线成功
32. 确认 TAPD 自动归档为「已完成」
33. 执行经验分层沉淀检查
34. 输出完整闭环报告
